...
const Otp = () => {
  ...
  try {
    const res = await axiosInstance.post('/auth/otp-confirm', {
      otp: code
    });

    if (res.data.success) {
      localStorage.removeItem('email'); // cuma bersihin email
      navigate('/login'); // atau redirect yang kamu mau
    }
  } catch (err) {
      ...
      <form onSubmit={verifyOtp} className="space-y-6">
        <div className="flex justify-center gap-3">
          {otp.map((digit, index) => (
            <input
              ...
              className="w-14 h-14 text-center text-2xl border rounded-lg focus:ring-2 focus:ring-kahf-green"
            />
          ))}
        </div>
        ...
        <div className="text-center text-gray-600">
          {timer > 0 ? (
            ...
            <span className="text-sm">Silakan register ulang</span>
          )}
        </div>
